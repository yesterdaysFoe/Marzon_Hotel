...
class PhpUploader {
    ...
    function GetString() {
        ...
        if ($this->MaxSizeKB > 0)
            $code.=$this->_GenerateAttribute("MaxSizeKB");

        if ($this->MaxFilesLimit > 0)
            $code.=$this->_GenerateAttribute("MaxFilesLimit");
    ...
    function _HandleXhttpUpload() {
        ...
        if (strpos($fn, "\0"))
            throw (new Exception("Invalid partial path !!"));
    ...
    function _HandleAddonUpload() {
        ...
        if (!$addonpartial)
            $addonpartial = @$_POST[PhpUploader_GetNamespace() . "_AjaxUploader_Partial"];
